...
class SearchOverlayState extends State<SearchOverlay>
      ...
      child: Material(
        ...
        child: ScaleTransition(
          ...
          child: SingleChildScrollView(
            child: Container(
              height: ScreenUtil.getHeight(context) / 0.9,
              color: Colors.white,
              child: Column(
                children: [
                  Container(
                            ...
                            child: TextFormField(
                              ...
                              onChanged: (string) {
                                ...
                                if (string.length >= 1) {
                                  API(context)
                                      .get(
                                    'productss/search?search=$string',
                                  )
                                      .then((value) {
                                    setState(() {
                                      loading = false;
      ...
      children: [
        Container(
          ...
          child: DropdownButton<dynamic>(
            ...
            items: [
              buildDropdown("product", getTransrlate(context, "product")),
              buildDropdown("sellers", getTransrlate(context, "sellers")),
              buildDropdown("brands", getTransrlate(context, "brands"))
            ],
            ...
            child: Center(
              ...
              onTap: () {
                showDialog(context: context, builder: (_) => Filterdialog())
                    .then((value) =>
                        {print(value), getSearch("${search_index}$value")});
              },
              child: Container(
                width: 50,
                child: Row(
        ...
        Container(
          ...
          child: DropdownButton<dynamic>(
            ...
            iconSize: ScreenUtil.getTxtSz(context, 14),
            underline: SizedBox(),
            value: _selectedSort,
            items: [
              buildDropdown(
                  "price_desc", getTransrlate(context, "SortByPricemore")),
              buildDropdown(
                  "price_asc", getTransrlate(context, "SortByPriceLess")),
              buildDropdown("date_asc", getTransrlate(context, "SortByold")),
              buildDropdown("date_desc", getTransrlate(context, "SortByNew"))
            ],
            onChanged: (selectedFilter) {
              setState(() {
  ...
  DropdownMenuItem buildDropdown(String value, String item) {
    ...
    if (search_index.length >= 1) {
      API(context)
        ...
        if (value != null) {
          if (value['status_code'] == 200) {
            setState(() {
              products = ProductModel.fromJson(value['data']).products;
            });
          } else {
            showDialog(
                context: context,
